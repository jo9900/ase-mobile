...
<template>
  <div class="page van-clearfix">
    ...
    <div class="page_apply_title" v-if="tableData.length > 0">
      <div class="page_apply_title_name">{{ $t("message.245") }}</div>
    ...
    <div class="wrap">
      <div class="page_content" :class="{ marginTop: tableData.length == 0 }">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span style="color: #104cff">{{ scope.row.username }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="rebate_level"
...
export default {
  ...
  methods: {
    ...
    get_list() {
      rebateLog(this.$qs.stringify(this.form)).then((res) => {
