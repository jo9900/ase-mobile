...
<template>
  <div class="page">
    ...
    <div class="page_apply_title" v-if="tableData.length > 0">
      <div class="page_apply_title_name">{{ $t("message.140") }}</div>
    ...
    <div class="wrap">
      <div class="page_content" :class="{ marginTop: tableData.length == 0 }">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span style="color: #104cff">{{ scope.row.name }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="invitee_count"
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.invitee_count != undefined">{{
                scope.row.invitee_count
              }}</span>
              <span v-if="scope.row.invitee_count == undefined">0</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="kyc_status "
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.kyc_status == 0">{{
                $t("message.97")
              }}</span>
              <span v-if="scope.row.kyc_status == 1">{{
                $t("message.101")
              }}</span>
              <span v-if="scope.row.kyc_status == 2">{{
                $t("message.102")
              }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    get_list() {
      marketingLog(this.$qs.stringify(this.form)).then((res) => {
